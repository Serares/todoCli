// we use a different package name from
// the one that we are testig
// to make sure we test only the exported functions/types
package todo_test
...
func TestAdd(t *testing.T) {
	...
	if l[0].Task != taskName {
		t.Errorf("Expected %s and got %s instead", taskName, l[0].Task)
	}
}
